...
void BossMap::Init()
{
	...
	stageBackground->GetComponent<Transform>()->SetLocalPosition(Vector2(1280.f, 1280.f));
	stageBackground->AddComponent<SpriteRenderer>();
	stageBackground->GetComponent<SpriteRenderer>()->SetSprite(GEngine->LoadSpriteFromSheet(L"..\\..\\4_Resources\\ProjectSheet\\BackgroundSheet\\Wall_Savepoint 20 x 20 (2560_2560).png", Vector2(0, 0), 2560.f, 2560.f, Vector2(0.5f, 0.5f), 0.f, 1.0f));
	stageBackground->GetComponent<SpriteRenderer>()->SetWH(2560.f, 2560.f);
	stageBackground->SetCameraView(true);

	MapLoader::LoadMap("MapData\\BossMap.json", cameraObj);
...
#pragma region UI
	{
		...
		// 게임 클리어 패널(보스방에서만 넣어주면 되겠다.
		GameObject* gameClearPanel = new GameObject(TAG::UI);
		gameClearPanel->SetName("GameClearPanel");
		gameClearPanel->AddComponent<Transform>();
		gameClearPanel->GetComponent<Transform>()->SetParent(canvas->GetComponent<Transform>());
		gameClearPanel->AddComponent<Panel>();
		gameClearPanel->GetComponent<Panel>()->SetSprite(GEngine->LoadSpriteFromSheet(L"..\\..\\4_Resources\\ProjectSheet\\UI\\GameClear UI\\GameClearPanel.png", Vector2(0, 0), 1280, 749, Vector2(0.5f, 0.5f), 1.0f, 1.0f));
		gameClearPanel->GetComponent<Panel>()->SetWH(1920.f, 1080.f);
		gameClearPanel->SetActive(false);
		gameClearPanel->SetCameraView(true);
		GET_SINGLE(UIManager)->SetInGameClearPanel(gameClearPanel);

		GameObject* clearUIText = new GameObject(TAG::UI);
		clearUIText->SetName("ClearUIText");
		clearUIText->AddComponent<Transform>();
		clearUIText->GetComponent<Transform>()->SetParent(gameClearPanel->GetComponent<Transform>());	// 버튼은 메뉴패널을 부모로 삼는다.
		clearUIText->GetComponent<Transform>()->SetLocalPosition(Vector2(0, -300.f));
		clearUIText->AddComponent<SpriteRenderer>();
		clearUIText->GetComponent<SpriteRenderer>()->SetSprite(GEngine->LoadSpriteFromSheet(L"..\\..\\4_Resources\\ProjectSheet\\UI\\GameClear UI\\ClearUI_Text.png", Vector2(0, 0), 1113, 347, Vector2(0.5f, 0.5f), 0.f, 1.0f));
		clearUIText->GetComponent<SpriteRenderer>()->SetWH(1113.f, 347.f);
		clearUIText->SetActive(false);
		clearUIText->SetCameraView(true);

		GameObject* clearUIMakerText = new GameObject(TAG::UI);
		clearUIMakerText->SetName("ClearUIMakerText");
		clearUIMakerText->AddComponent<Transform>();
		clearUIMakerText->GetComponent<Transform>()->SetParent(gameClearPanel->GetComponent<Transform>());	// 버튼은 메뉴패널을 부모로 삼는다.
		clearUIMakerText->GetComponent<Transform>()->SetLocalPosition(Vector2(0, 100.f));
		clearUIMakerText->AddComponent<SpriteRenderer>();
		clearUIMakerText->GetComponent<SpriteRenderer>()->SetSprite(GEngine->LoadSpriteFromSheet(L"..\\..\\4_Resources\\ProjectSheet\\UI\\GameClear UI\\ClearUI_Ending.png", Vector2(0, 0), 1091, 475, Vector2(0.5f, 0.5f), 0.f, 1.0f));
		clearUIMakerText->GetComponent<SpriteRenderer>()->SetWH(1091.f, 475.f);
		clearUIMakerText->SetActive(false);
		clearUIMakerText->SetCameraView(true);

		GameObject* clearUIThanksText = new GameObject(TAG::UI);
		clearUIThanksText->SetName("ClearUIText");
		clearUIThanksText->AddComponent<Transform>();
		clearUIThanksText->GetComponent<Transform>()->SetParent(gameClearPanel->GetComponent<Transform>());	// 버튼은 메뉴패널을 부모로 삼는다.
		clearUIThanksText->GetComponent<Transform>()->SetLocalPosition(Vector2(0, 400.f));
		clearUIThanksText->AddComponent<SpriteRenderer>();
		clearUIThanksText->GetComponent<SpriteRenderer>()->SetSprite(GEngine->LoadSpriteFromSheet(L"..\\..\\4_Resources\\ProjectSheet\\UI\\GameClear UI\\ClearUI_Thank_Text.png", Vector2(0, 0), 1149, 93, Vector2(0.5f, 0.5f), 0.f, 1.0f));
		clearUIThanksText->GetComponent<SpriteRenderer>()->SetWH(1149.f, 93.f);
		clearUIThanksText->SetActive(false);
		clearUIThanksText->SetCameraView(true);
